...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 z-50 h-screen w-72 bg-[#00004d] text-white p-8
          rounded-r-[2rem] shadow-2xl transform transition-transform duration-300
      ...
      >
        ...
        <nav className="space-y-4">
          {menuItems.map((item) => (
            <Link
              ...
              ${
                pathname === item.href
                  ? "bg-white text-[#00004d] shadow-xl scale-105"
                  : "hover:bg-blue-900/40 text-blue-100"
              }`}
            >
              <item.icon size={20} />
              {item.name}
        ...
        <button
          ...
          className="absolute bottom-10 left-8 right-8 p-4 rounded-2xl bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-3 font-black text-[10px] tracking-widest"
        >
          <LogOut size={16} />
          LOGOUT
        </button>
